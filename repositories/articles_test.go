...
import (
  ...
  "github.com/cho6in/myapi/repositories/testdata"
...
func TestInsertArticle(t *testing.T) {
  ...
  if newArticle.ID != expectedNum {
    t.Errorf("new article id is expected %d but got %d\n", expectedNum, newArticle.ID)
  }

  t.Cleanup(func() {
    const sqlStr = `
      delete from article where title = ? and contents = ? and username = ?
    `
...
func TestUpdateNiceNum(t *testing.T) {
  ...
  if got.NiceNum-testdata.ArticleTestData[articleID-1].NiceNum != 1 {
    t.Errorf("fail to update nice num: expected %d but got %d\n",testdata.ArticleTestData[articleID].NiceNum, got.NiceNum)
  }
}
